...
#
# PROGRESS LOGGING: since each replicate runs on a separate parallel
# worker process, cat()/print() calls inside the %dopar% block do NOT
# appear in this console. Instead, each replicate appends one line to
# output/bootstrap_progress.log as soon as it finishes. While the job
# is running, open a separate R session or terminal and run:
#     cat(readLines("output/bootstrap_progress.log"), sep = "\n")
# or, from a terminal: tail -f output/bootstrap_progress.log
#
...
# ------------------------------------------------------------
# 0. SET UP PARALLEL CLUSTER
# ------------------------------------------------------------
cores <- max(1, detectCores() - 8)   # guarded so this can't go to 0/negative
...
# ------------------------------------------------------------
lambda                  <- 1   # <<== glmmLasso penalty -- set/tune here
...
# progress log file -- deleted/recreated fresh at the start of each run
progress_log <- "output/bootstrap_progress.log"
...
cat(sprintf("Progress will be written to: %s\n", progress_log))
...
boot_raw_results <- foreach(
  ...
  .export   = c("df", "all_subject_ids", "sample_frac", "SAMPLE_WITH_REPLACEMENT",
                "n_imputations", "mice_maxit", "impute_subset", "fit_lasso_lmm",
                "lambda", "fix_formula", "rnd_structure", "response_var",
                "level1_vars_continuous", "level1_vars_binary",
                "level2_vars_continuous", "level2_vars_categorical",
                "all_continuous", "redundant_dupes", "progress_log"),
  .options.RNG = TRUE
) %dopar% {

  replicate_start_time <- Sys.time()
  ...
  # --- Step 3: log progress for this replicate (workers don't print to console) ---
  replicate_secs <- as.numeric(difftime(Sys.time(), replicate_start_time, units = "secs"))

  log_line <- if (boot_result$success) {
    sprintf("[%s] Replicate %d/%d done in %.1fs -- avg RSS = %.3f",
            format(Sys.time(), "%H:%M:%S"), b, n_bootstrap,
            replicate_secs, boot_result$avg_rss)
  } else {
    sprintf("[%s] Replicate %d/%d FAILED after %.1fs -- %s",
            format(Sys.time(), "%H:%M:%S"), b, n_bootstrap,
            replicate_secs, boot_result$error_msg)
  }
...
# --- Post-Processing: reconstruct status vectors/lists from parallel output ---
...
# lambda is folded into the filename (dots replaced with "p" so the
# value stays readable in a filesystem path, e.g. lambda = 0.1 -> "0p1")
lambda_tag  <- gsub("\\.", "p", as.character(lambda))
output_file <- sprintf("output/bootstrap_lasso_results_map_lambda_%s.rds", lambda_tag)

saveRDS(bootstrap_lasso_results_map, file = output_file)
cat(sprintf("\nSaved bootstrap_lasso_results_map -> %s\n", output_file))