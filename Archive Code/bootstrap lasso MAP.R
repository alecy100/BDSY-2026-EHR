...
# ------------------------------------------------------------
lambda                  <- 10.0   # <<== glmmLasso penalty -- set/tune here
...
boot_raw_results <- foreach(
  ...
  .packages = c("dplyr", "mice"), # Add any other packages required by your functions
  .options.RNG = TRUE             # Safely manages seed generation across workers
) %dopar% {
