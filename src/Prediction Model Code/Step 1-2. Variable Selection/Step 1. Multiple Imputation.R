...
#   - reads/writes prediction/ instead of output/
#   - "cpp" removed from level1_vars_continuous (no longer in cleanData.csv)
#   - "sbp"/"dbp" added to level1_vars_continuous so they get proper
#     multilevel (2l.pmm) imputation now that they're eligible LMM
#     candidates (see utils_variable_pool.R) instead of falling back to
#     mice's single-level default method
#   - the is_missing_* never_impute filter was removed since those
#     indicator columns no longer exist in cleanData.csv
#
# Produces 5 (train, test) pairs: prediction/imputations/imputed_pair_1.rds ... _5.rds
# ============================================================
...
split <- readRDS("prediction/train_test_split.rds")
...
level1_vars_continuous <- c("heart_rate", "respiratory_rate", "spo2",
                             "gcs", "sbp", "dbp", "rass", "temperature_c", "bun",
...
dir.create("prediction/imputations", showWarnings = FALSE, recursive = TRUE)
...
for (k in seq_len(n_imputations)) {
  ...
  saveRDS(list(train = train_imp, test = test_imp),
          file = sprintf("prediction/imputations/imputed_pair_%d.rds", k))
